...
function LabeledInput({
  ...
  return (
    <div className="space-y-3 w-full">
      <label
        className={`text-base sm:text-lg font-semibold ${isLight ? "text-gray-700" : "text-white/90"
          } ${isFa ? "font-vazirmatn" : ""
          }`}
      >
        {label}
      ...
      <div className="relative">
        {/* +98 Prefix */}
        <span
          className={`absolute left-4 top-1/2 -translate-y-1/2 text-base sm:text-lg font-semibold ${isLight ? "text-gray-600" : "text-white/70"
            } pointer-events-none`}
          dir="ltr"
        >
          +98
        ...
        <input
          ...
          dir="ltr"
          className={`h-12 sm:h-14 w-full rounded-md sm:rounded-xl pl-16 pr-4 text-base sm:text-lg transition-all duration-300 ${error
      ...
      {error && (
        <p className={`text-sm ${isLight ? "text-red-600" : "text-red-400"} ${isFa ? "font-vazirmatn" : ""}`}>
          {error}
        </p>
      )}
    </div>
...
> = {
  [Language.EN]: {
    ...
    errorInvalid: "Please enter a valid Iranian phone number (9 digits starting with 9)",
  ...
  [Language.FA]: {
    ...
    brandName: "Barbod",
    phonePlaceholder: "9123456789",
    errorInvalid: "لطفاً یک شماره تلفن معتبر وارد کنید (9 رقم که با 9 شروع شود)",
    errorRequired: "شماره تلفن الزامی است",
  },
};
...
const validateIranianPhone = (phone: string): boolean => {
  ...
  return /^9[0-9]{9}$/.test(normalized);
};
