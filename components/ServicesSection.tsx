...
const servicesData: Record<Language, Service[]> = {
  [Language.EN]: [
    ...
    },
    {
      ...
      description: "Phrase-independent voice authentication for secure, frictionless phone banking and support.",
  ...
  [Language.FA]: [
    {
      ...
      description: "تطبیق چهره ۱:۱ و ۱:N با تحلیل سه‌بعدی؛ مقاوم در برابر نور، سن و اکسسوری.",
      imageGradient: "linear-gradient(135deg, #06b6d4 0%, #3b82f6 100%)",
      colorFrom: "#06b6d4",
      colorTo: "#3b82f6",
      icon: <ScanFace className="w-6 h-6" />,
    ...
    },
    {
      id: "ocr",
      title: "OCR مدارک",
      description: "استخراج داده آنی از مدارک هویتی با پشتیبانی فارسی، عربی و لاتین.",
      imageGradient: "linear-gradient(135deg, #0ea964 0%, #22d3ee 100%)",
      colorFrom: "#0ea964",
      colorTo: "#22d3ee",
      icon: <FileText className="w-6 h-6" />,
    },
    {
      id: "voice",
      title: "بیومتریک صوتی",
      description: "احراز هویت صوتی مستقل از عبارت؛ مناسب مراکز تماس و بانکداری تلفنی.",
      imageGradient: "linear-gradient(135deg, #f59e0b 0%, #ef4444 100%)",
      colorFrom: "#f59e0b",
      colorTo: "#ef4444",
      icon: <Mic className="w-6 h-6" />,
...
const titles: Record<Language, { heading: string; sub: string; button: string }> = {
  [Language.EN]: {
            ...
            return (
              ...
              >
                <div className="flex items-center gap-4 relative z-10" dir={dir}>
                  ...
                  <span
                    className={`font-semibold transition-colors duration-300 ${isActive ? "text-[color:var(--md-sys-color-on-surface)]" : "text-[color:var(--md-sys-color-on-surface-variant)]"
                      }`}
                  >
                    {service.title}
                  </span>
                ...
                <div className="absolute bottom-0 left-0 right-0 h-1.5 bg-transparent">
                  <div
                    key={isActive ? `progress-${activeId}` : `progress-${service.id}`}
                    className="h-full rounded-full"
                    style={{
                      opacity: isActive ? 1 : 0,
                      background: `linear-gradient(90deg, ${service.colorFrom}, ${service.colorTo})`,
                      animation: isActive ? `serviceProgress ${PROGRESS_DURATION}ms linear forwards` : "none",
                    }}
                  />
                </div>
              </button>
                        ...
                        {[...Array(20)].map((_, i) => {
                          ...
                          return (
                            <motion.rect
                              ...
                              style={{ transformOrigin: `${10 + i * 19 + 6}px 50px` }}
                            />
                          );
                        })}
                      </svg>
