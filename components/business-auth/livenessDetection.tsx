...
import { LivenessDetectionPayload } from "../../types/businessProfile";

interface LivenessDetectionProps {
  onBack?: () => void;
  onContinue?: (data: LivenessDetectionForm) => void;
  onSkip?: () => void;
  initialData?: LivenessDetectionPayload;
  isLoading?: boolean;
}

type LivenessDetectionForm = LivenessDetectionPayload;
...
export function LivenessDetection({ onBack, onContinue, onSkip, initialData, isLoading }: LivenessDetectionProps) {
    ...
    >
      ...
      <div className="relative space-y-5">
        ...
        <div className="space-y-2 text-right">
          ...
          >
            برای فعال سازی سرویس تشخیص زنده‌بودن چهره، اطلاعات زیر را وارد کنید.
          </Typography>
        </div>
        ...
        <div className="flex justify-right">
          <label className="flex items-center gap-3 rounded-2xl bg-[color:var(--md-sys-color-surface-container)] px-4 py-3 text-[color:var(--md-sys-color-on-surface)] shadow-[var(--elevation-1)] ring-1 ring-[color:var(--md-sys-color-outline-variant)]">
            ...
            />
            <span className="text-sm sm:text-base font-semibold">درخواست فعال سازی سرویس تشخیص زنده‌بودن چهره را دارم.</span>
          </label>
        </div>
