...
const sheetVariants = cva(
  "fixed z-50 gap-4 bg-[color:var(--md-sys-color-surface)] p-6 shadow-[var(--elevation-3)] transition ease-in-out",
  {
    variants: {
      side: {
...
const SheetHeader = ({ className, ...props }: React.HTMLAttributes<HTMLDivElement>) => (
  <div className={cn("flex flex-col gap-1.5 text-center sm:text-right", className)} {...props} />
);
...
>(({ className, ...props }, ref) => (
  <SheetPrimitive.Title
    ref={ref}
    className={cn("text-lg font-semibold text-[color:var(--md-sys-color-on-surface)]", className)}
    {...props}
  />
));
...
>(({ className, ...props }, ref) => (
  <SheetPrimitive.Description
    ref={ref}
    className={cn("text-sm text-[color:var(--md-sys-color-on-surface-variant)]", className)}
    {...props}
  />
));
